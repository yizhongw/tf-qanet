...
class BidirectionalLanguageModel(object):
    def __init__(
        ...
        Three options for input type:
            ...
            (3) To use precomputed word embedding as input,
                pass use_precomputed_inputs as true and ids_placeholder
                of shape (None, None, word_embedding_dim) to __call__.
                This will override use_character_inputs.
        ...
        use_precomputed_inputs: if True, use precomputed word embedding as input,
            will override use_character_inputs
        max_batch_size: the maximum allowable batch size,
        override_options: if not None, will override options from options_file,
        char_part_only: if True, will only compute character to word embedding
        '''
        ...
        self._use_precomputed_inputs = use_precomputed_inputs
        self._char_part_only = char_part_only

        self._ops = {}
        self._graphs = {}

    def __call__(self, ids_placeholder, length_placeholder=None):
        '''
        ...
        '''
        if self._use_precomputed_inputs and length_placeholder is None:
            raise ValueError('must pass length_placeholder as input '
                            'when use precomputed input')
        if ids_placeholder in self._ops:
            ...
            if len(self._ops) == 0:
                ...
                lm_graph = BidirectionalLanguageModelGraph(
                    ...
                    use_precomputed_inputs=self._use_precomputed_inputs,
                    max_batch_size=self._max_batch_size,
                    char_part_only=self._char_part_only,
    ...
    def _build_ops(self, lm_graph):
        dependencies = [] if self._options.get('use_transformer', False) or self._char_part_only else [lm_graph.update_state_op]
        with tf.control_dependencies(dependencies):
            # get the LM embeddings
            token_embeddings = lm_graph.embedding
            if self._char_part_only:
                return {'token_embeddings': token_embeddings}
            layers = [
...
class BidirectionalLanguageModelGraph(object):
    ...
    def __init__(self, options, weight_file, ids_placeholder,
        ...
        self.char_part_only = char_part_only
        self.length_placeholder = length_placeholder

        if length_placeholder is None and use_precomputed_inputs:
            raise ValueError('must pass length_placeholder when use precomputed inputs')
    ...
    def _build(self):
        if self.use_precomputed_inputs:
            self._build_precomputed_embeddings()
        elif self.use_character_inputs:
            self._build_word_char_embeddings()
        else:
            self._build_word_embeddings()
        if self.options.get('scale_embeddings', False):
            self.embedding = self.embedding * (tf.shape(self.embedding)[2] ** 0.5)
        if self.char_part_only:
            return
    ...
    def _build_precomputed_embeddings(self):
        self.embedding = self.ids_placeholder
    ...
    def _build_lstms(self):
        ...
        if self.use_precomputed_inputs:
            xbatchsize  = tf.shape(self.embedding)[0]
            max_len = tf.shape(self.embedding)[1]
            xmask = tf.reshape(tf.tile(tf.range(max_len), [xbatchsize]),
                                shape=[xbatchsize, max_len])
            mask = xmask < tf.reshape(self.length_placeholder, shape=[-1,1])
        elif self.use_character_inputs:
    ...
    def _build_transformers(self):
        ...
        att_bias = attention_bias(tf.shape(self.embedding)[1], "causal")
        ...
        for direction in ['forward', 'backward']:
            ...
            layer_input = add_timing_signal(layer_input)
            lstm_num = 0 if direction == 'forward' else 1
            layer_outputs = []
            with tf.control_dependencies([layer_input]):
                with tf.variable_scope("transformer_%s" % lstm_num):
                    lstm_output = transformer_decoder(
                        layer_input,
                        None,
                        att_bias,
                        None,
