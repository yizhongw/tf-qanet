...
class Vocabulary(object):
    ...
    def __init__(self, filename, validate_file=False):
        ...
        if filename is None:
            self._id_to_word = ['</S>', '<S>', '<UNK>']
            self._word_to_id['</S>'] = 0
            self._word_to_id['<S>'] = 1
            self._word_to_id['<UNK>'] = 2
            self._unk = 2
            self._bos = 1
            self._eos = 0
            return
...
##### for training
def _get_batch(generator, batch_size, num_steps, max_word_length, num_context_steps):
    """Read batches of input."""
    cur_streams = [None] * batch_size
    if num_context_steps < 0:
        num_context_steps = 0

    no_more_data = False
    inputs = np.zeros([batch_size, num_steps + num_context_steps], np.int32)
    context = np.zeros([1, num_context_steps], np.int32)
    if max_word_length is not None:
        char_inputs = np.zeros([batch_size, num_steps + num_context_steps, max_word_length],
                            np.int32)
        char_context = np.zeros([1, num_context_steps, max_word_length], np.int32)
    else:
        char_inputs = None
    targets = np.zeros([batch_size, num_steps], np.int32)

    cur_pos = 0
    cur_stream = cur_streams[0]
    while cur_pos < num_context_steps:
        if cur_stream is None or len(cur_stream[0]) <= 1:
            try:
                cur_streams[0] = list(next(generator))
                cur_stream = cur_streams[0]
            except StopIteration:
                no_more_data = True
                break
        how_many = min(len(cur_stream[0]) - 1, num_context_steps - cur_pos)
        next_pos = cur_pos + how_many
        context[0, cur_pos:next_pos] = cur_stream[0][:how_many]
        if max_word_length is not None:
            char_context[0, cur_pos:next_pos] = cur_stream[1][:how_many]
        cur_pos = next_pos
        cur_stream[0] = cur_stream[0][how_many:]
        if max_word_length is not None:
            cur_stream[1] = cur_stream[1][how_many:]
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            if num_context_steps <= 0:
                stream_id = i
            cur_stream = cur_streams[stream_id]
            if num_context_steps > 0:
                inputs[i, :num_context_steps] = context[0]
                if max_word_length is not None and max_word_length > 0:
                    char_inputs[i, :num_context_steps] = char_context[0]
            while cur_pos < num_steps:
                if cur_stream is None or len(cur_stream[0]) <= 1:
                    try:
                        cur_streams[stream_id] = list(next(generator))
                        cur_stream = cur_streams[stream_id]
                    except StopIteration:
                ...
                how_many = min(len(cur_stream[0]) - 1, num_steps - cur_pos)
                next_pos = cur_pos + how_many

                inputs[i, (cur_pos + num_context_steps):(next_pos + num_context_steps)] = cur_stream[0][:how_many]
                if max_word_length is not None:
                    char_inputs[i, (cur_pos + num_context_steps):(next_pos + num_context_steps)] = cur_stream[1][:how_many]
                targets[i, cur_pos:next_pos] = cur_stream[0][1:how_many+1]
            ...
            seq_length[i] = cur_pos
            if (not no_more_data) and num_context_steps > 0:
                context[0,:] = inputs[i, -num_context_steps:]
                if max_word_length is not None and max_word_length > 0:
                    char_context[0,:] = char_inputs[i, -num_context_steps:]
        ...
        X = {'token_ids': inputs, 'tokens_characters': char_inputs,
                 'next_token_id': targets, 'seq_length': seq_length}
...
class LMDataset(object):
    ...
    def __init__(self, filepattern, vocab, reverse=False, test=False,
                 shuffle_on_load=False, num_context_steps=0):
        '''
        ...
        self._num_context_steps = num_context_steps 

        self._ids = self._load_random_shard()
    
    def reset(self):
        self._all_shards = glob.glob(self._filepattern)
        self._shards_to_choose = []
        self._ids = self._load_random_shard()
    ...
    def iter_batches(self, batch_size, num_steps):
        for X in _get_batch(self.get_sentence(), batch_size, num_steps,
                           self.max_word_length, self._num_context_steps):
...
class BidirectionalLMDataset(object):
    ...
    def reset(self):
        self._data_forward.reset()
        self._data_reverse.reset()

    def iter_batches(self, batch_size, num_steps, num_context_steps=0):
        max_word_length = self._data_forward.max_word_length

        data = zip(
            _get_batch(self._data_forward.get_sentence(), batch_size,
                  num_steps, max_word_length, num_context_steps),
